...
def setup_logging():
    ...
    if not logging.getLogger().handlers:
        ...
        # Reduce the noise from specific libraries
        logging.getLogger('PIL.TiffImagePlugin').setLevel(logging.WARNING)
        logging.getLogger('primp.utils').setLevel(logging.INFO)
...
class SearchTool:
    ...
    def _transform_query(self, conversation: str, question: str) -> str:
        """Transform a question into a search-friendly query using LLM, with conversation context."""
        try:
            prompt = f"""Given the following conversation history and the latest user question, transform the question into an optimal search query for a search engine.
        ...
        3. Add necessary and valid search operators or modifiers 
    ...
    def run(self, messages: List[Dict[str, str]], num_results: int = 12) -> List[Dict[str, str]]:
        """Perform an internet search with rate limiting, using conversation context."""
        try:
            # Build conversation and get latest user question
            conversation = "\n".join([f"{msg['role']}: {msg['content']}" for msg in messages])
            user_messages = [msg["content"] for msg in messages if msg["role"] == "user"]
            latest_question = user_messages[-1] if user_messages else ""
            search_query = self._transform_query(conversation, latest_question)
...
class Agent:
    ...
    def _should_search(self, conversation: str, question: str) -> bool:
        """Use the LLM to determine if a search is needed, considering full context."""
        try:
            prompt = f"""Given the following conversation history and the latest user question, determine if the question requires searching the internet for current information.
    ...
    Answer (yes/no):"""
            ...
            logger.debug(f"Search decision for '{question}': {decision}")
            return decision == "yes"
        except Exception as e:
    ...
    def _should_search_node(self, state: AgentState) -> AgentState:
        """Node to determine if a search is needed, using full conversation context and latest user question."""
        try:
            ...
            latest_question = user_messages[-1] if user_messages else ""
            should_search = self._should_search(conversation, latest_question)
    ...
    def _perform_search(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            results = self.search_tool.run(messages)
    ...
    def _generate_response(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            messages = state["messages"]
            search_results = state.get("search_results") # Use .get() for safety
